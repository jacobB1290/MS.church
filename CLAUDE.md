...
| `search-harness.mjs` (separate file, `npm run search:test`) | The /watch library client search. Pulls the **LIVE** search index off the deployed page and runs the same ranking the browser does, so relevance is tuned against real published content. Data-derived: it auto-generates checks from the catalog (every topic ranks its item in the top 3, every song title returns its song, every speaker recalls their messages, title boilerplate stays quiet, and the live→love fuzzy-collision guard), so it adapts as the library grows — no hardcoded dates. Ad-hoc mode prints rankings for any query: `node scripts/search-harness.mjs "joy"`. Override the source with `HARNESS_URL=…`. **The scoring + title-cleaning in the script are a verbatim copy of the search IIFE in `src/templates/watch-body.ts` — tune both together** (same idiom as `calendar.ts` ↔ `eventMapping.ts`). |

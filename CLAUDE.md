...
| `search-harness.mjs` (separate file, `npm run search:test`) | The /watch library client search. Pulls the **LIVE** search index off the deployed page and runs the same ranking the browser does, so relevance is tuned against real published content. Data-derived: it auto-generates checks from the catalog (every topic ranks its item in the top 3, every song title — or a title twin — returns its song, every speaker recalls their messages, title boilerplate doesn't flood, and the live→love fuzzy-collision guard), so it adapts as the library grows — no hardcoded dates. It also covers the **expanded** searchable dimensions: scripture (every cited book findable + abbreviations like `mt`/`rom`, `matthew 6` chapter precision, `1 john` numbered-book disambiguation), dates (month/year/day), distinctive chapters (`testimony`/`benediction`/…), and the owner's **precision guard** that an incidental summary/chapter-only mention never outranks a topical match. New-dimension checks skip cleanly until the enriched index deploys. Ad-hoc mode prints rankings for any query: `node scripts/search-harness.mjs "romans 8"`. Override the source with `HARNESS_URL=…`. **The scoring + title-cleaning in the script are a verbatim copy of the search IIFE in `src/templates/watch-body.ts` — tune both together** (same idiom as `calendar.ts` ↔ `eventMapping.ts`); the index BUILDER (scripture/date/chapter expansion) lives only in `watch-body.ts`. |
...
- Serverless function: `api/index.ts`. **Do NOT simplify this back to `export default handle(app)`.** The bare `@hono/node-server/vercel` `handle()` hangs on any request with a body on Vercel's Node runtime — the body stream never reaches Hono's `c.req.json()`, so POST `/api/contact` dies at the function timeout (`FUNCTION_INVOCATION_TIMEOUT`, honojs/node-server#306). `api/index.ts` therefore keeps `handle()` for bodyless GET/HEAD and uses a small custom bridge for body methods: buffer the body (or reuse Vercel's pre-parsed `req.body`), build a Web `Request`, call `app.fetch()`, write the `Response` back. The Vite dev server does NOT exercise `api/index.ts`, so this path is only testable on a real Vercel deploy.
